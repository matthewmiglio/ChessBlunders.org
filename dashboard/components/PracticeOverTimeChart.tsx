...
export function PracticeOverTimeChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const res = await fetch("/api/usage/practice-daily");
  ...
  if (data.length === 0) {
    return (
      <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
        <h3 className="text-sm font-bold text-gray-100 mb-4">Blunders Practiced Over Time</h3>
  ...
  const total = data.reduce((sum, d) => sum + d.practice_count, 0);
  ...
  const points = data.map((d, i) => {
    ...
    const y = chartHeight - (d.practice_count / maxCount) * chartHeight;
    return { x, y, data: d };
  });
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-gray-100">Blunders Practiced Over Time</h3>
          ...
          >
            ...
            {points.map((p, i) => (
              <g key={i} className="group">
                ...
                />
                <title>{`${formatDateEST(p.data.day)}: ${p.data.practice_count} positions`}</title>
              </g>
            ))}
          </svg>
        </div>
