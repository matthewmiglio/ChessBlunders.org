...
export function EngineUsageOverTime() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const res = await fetch("/api/usage/engine-daily");
  ...
  const points = data.map((d, i) => {
    ...
    const y = chartHeight - (d.request_count / maxCount) * chartHeight;
    return { x, y, data: d };
  });
          ...
          >
            ...
            {points.map((p, i) => (
              <g key={i} className="group">
                ...
                />
                <title>{`${formatDateEST(p.data.day)}: ${p.data.request_count.toLocaleString()} calls`}</title>
              </g>
            ))}
          </svg>
        </div>
