...
export function GamesOverTimeChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const res = await fetch("/api/usage/games-daily");
  ...
  if (data.length === 0) {
    return (
      <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
        <h3 className="text-sm font-bold text-gray-100 mb-4">Games Over Time</h3>
        <p className="text-gray-500 text-sm">No data available</p>
      </div>
    );
  }

  const maxCount = Math.max(...data.map((d) => d.game_count), 1);
  const total = data.reduce((sum, d) => sum + d.game_count, 0);
  ...
  const points = data.map((d, i) => {
    ...
    const y = chartHeight - (d.game_count / maxCount) * chartHeight;
    return { x, y, data: d };
  });
  ...
  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-gray-100">Games Over Time</h3>
      ...
      <div className="flex">
        ...
        <div className="flex-1 relative" style={{ height: `${chartHeight}px` }}>
          ...
          >
            ...
            {/* Area fill */}
            <path d={areaPath} fill="rgba(14, 165, 233, 0.1)" />
            {/* Line */}
            <path
              d={linePath}
              fill="none"
            ...
            {points.map((p, i) => (
              <g key={i} className="group">
                ...
                />
                <title>{`${formatDateEST(p.data.day)}: ${p.data.game_count} games`}</title>
              </g>
            ))}
          </svg>
        </div>
